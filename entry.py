...
def gc_as_lib(argv):
    ...
    env = HoK1v1.load_game(runtime_id=actor_id,
                           ...
                           eval_mode=False, config_path="config.dat", remote_param=remote_param)

    for i in range(AGENT_NUM):
        dataset_f = '{}-{}-{}.h5'.format(FLAGS.gamecore_ip, actor_id, AGENT_NUM)
        # ("frame_no", "observation", "legal_action", "action", "reward", "sub_action", "done")
        agents.append(Agent(
            Model, FLAGS.model_pool_addr.split(";"),
            keep_latest=(i==main_agent and not eval_mode),
            local_mode=eval_mode,
            # dataset=dataset_f  # comment this out when collecting data
            ))

    sample_manager = SampleManager(mem_pool_addr=FLAGS.mem_pool_addr, mem_pool_type="zmq",

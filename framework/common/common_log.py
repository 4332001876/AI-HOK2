...
class CommonLogger:
    ...
    def init_config():
        ...
        #CommonLogger.g_log_instance = logging.getLogger()
    ...
    def set_config(actor_id):
        formatter = logging.Formatter(fmt='%(asctime)s %(levelname)s %(filename)s[line:%(lineno)d]\
                %(module)s[%(funcName)s] %(thread)d %(message)s')

        # set file handler: file rotates with time
        filename = ("../log/info_%d.log" % int(actor_id))
        rf_handler = logging.handlers.TimedRotatingFileHandler(filename=filename, when='H', interval=3, backupCount=1)
